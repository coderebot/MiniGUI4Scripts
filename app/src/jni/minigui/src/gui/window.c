/* 
 ** $Id: window.c 13777 2011-07-21 05:59:40Z xwyan $
 **
        ...
        HDC real_dc, const RECT* rc, DWORD flags)
{
    ...
    if (flags == HT_CLIENT){
        GetClientRect ((HWND)pWin, &wnd_rc);
    }
    else {
        GetWindowRect ((HWND)pWin, &wnd_rc);
    }
    ...
    if (flags == HT_CLIENT) {
        ...
        while (child) {
            ...
            if ((child->dwStyle & WS_VISIBLE)
                ...
                RECT rcTmp;
                GetClientRect ((HWND)child, &rcTmp);
        ...
        BOOL hasMenu)
{
    ...
    int GUIAPI 
ClientWidthToWindowWidthEx (DWORD dwStyle, int win_type, int cw)
{
    int width = cw;
    const WINDOW_ELEMENT_RENDERER* rdr = 
    ...
    int GUIAPI 
ClientHeightToWindowHeightEx (DWORD dwStyle, int win_type, 
        int ch, BOOL hasMenu)
{
    int height = ch;
    const WINDOW_ELEMENT_RENDERER* rdr = 
        ...
        const RECT* rc1, const RECT* rc2)
{
    ...
    //BUFIX: if we scrolled all area, the all area must
    //be set as invalidate (dongjunjie 2010/7/28)
    if((iOffx != 0 && abs(iOffx) >= RECTW(rcScroll))
        || (iOffy != 0 && abs(iOffy) >= RECTH(rcScroll)))
    {
    ...
    if (IsControl (hWnd)) {
        ...
        if (!(pCtrl->dwExStyle & WS_EX_CTRLASMAINWIN)) {
            ...
            if (pCtrl->mask_rects == NULL) {
                ...
                if (!pCtrl->mask_rects) {
#ifdef _DEBUG
                    fprintf (stderr, "GUI>Window: Not enough memory!\n");
#endif
                    free (rect);
                    return FALSE;
                }
